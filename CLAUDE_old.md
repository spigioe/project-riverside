...
- 21. lépés KÉSZ: Merge UX javítás — forrás-elválasztó az üzenetszálban, "Összevonva" banner + readonly
  ...
  - UTÓLAGOS JAVÍTÁS (a felhasználó "nem javult" visszajelzése után, böngészőben tesztelve általa): a
    fenti 21. lépés elválasztó-implementációja EGY VALÓDI REGRESSZIÓT okozott a kimenő buborékok
    jobbra-igazításában — a `messages.map`-ben minden üzenetet egy sima (osztály nélküli) `<div>`-be
    csomagoltam az elválasztó elhelyezéséhez. Emiatt a `.messageRow` (amin az `.outbound { align-self:
    flex-end }` szabály ül) már NEM közvetlen gyereke a `.thread` flex konténernek — `align-self`
    viszont KIZÁRÓLAG akkor hat, ha az elem közvetlen flex-item a szülőjében. A sima `<div>` szülő
    miatt a `.messageRow` visszaesett normál block-doboz viselkedésre (bal oldalra igazítva, mind
    inbound, mind outbound esetén) — EZ volt az igazi ok amiért a korábbi `.bubble` `max-width`/
    `word-break` CSS-módosítás önmagában nem oldotta meg a problémát, a tényleges hiba strukturális
    volt, nem CSS-érték probléma. JAVÍTVA: `MessageThread.tsx` — a wrapper `<div>` helyett `<Fragment
    key={msg.id}>` (React `Fragment`, nem renderel DOM elemet), így az elválasztó ÉS a `.messageRow`
    is visszakerült közvetlen flex-item pozícióba a `.thread` alatt, az `align-self` ismét működik
  - MÁSODIK, FÜGGETLEN HIBA (ugyanebben a felhasználói visszajelzésben): "a beérkező üzenetek nem
    látszódnak a threadben" — ez NEM rendering-hiba volt, hanem egy hiányzó adat: a ticket LÉTREHOZÁSA
    (mindhárom úton: Portal `POST /tickets`, Developer API `POST /api/v1/tickets`, és email-eredetű
    ticketek CSATOLMÁNY NÉLKÜL) SOHA nem hozott létre egy kezdő `TicketMessage` sort a kérelmező eredeti
    üzenetéhez — a tartalom KIZÁRÓLAG `Ticket.Body`-ban létezett, amit sem a `MessageThread`, sem a
    `TicketInfoPanel` nem jelenít meg (ez a 19. lépésben már dokumentált, tudatos minimalizálás volt,
    de a felhasználó most explicit hibaként azonosította — jogosan, mert így egy friss, válasz nélküli
    ticketnél a "Beszélgetés" panel teljesen ÜRES volt, holott a kérelmező eredeti panasza létezik).
    ELLENŐRIZVE élő DB-n: `SELECT Direction, COUNT(*) FROM TicketMessages GROUP BY Direction` → a
    meglévő 29 üzenetből mindössze 2 volt Inbound (egyetlen ticketen, #8, ahol egy KORÁBBI email-válasz
    hozta létre őket) — az összes többi ticket eredeti kérelmező-üzenete csak `Ticket.Body`-ban volt
    JAVÍTVA:
    - `TicketService.CreateTicketAsync`: a ticket mentése UTÁN mindig létrejön egy kezdő
      `TicketMessage` (`Direction=Inbound`, `SenderEmail=ticket.RequesterEmail`, `Body=ticket.Body`) —
      ez FEDI mindhárom portál/API létrehozási utat, mert mindkét controller (`TicketController`,
      `TicketsV1Controller`) ugyanezt a service-metódust hívja
    - `TicketEmailProcessor.ProcessOneAsync`: az "csak akkor hozunk létre kezdő üzenetet, ha van
      csatolmány" feltétel (19. lépés, eredeti indoklás: a csatolmánynak kell egy `TicketMessage.Id`)
      MEGSZÜNTETVE — a kezdő bejövő `TicketMessage` MOSTANTÓL MINDIG létrejön új ticketnél, a
      csatolmány-feltöltés (`UploadAttachmentsAsync`) változatlanul csak akkor fut, ha ténylegesen van
      csatolmány, de már az always-created message Id-jára hivatkozva
    - NEM migráció — a `TicketMessage` tábla már eleve támogatta ezt, nem kellett séma-változtatás
    - ADAT-BACKFILL a dev DB-n (egyszeri, közvetlen SQL, `docker exec sp_db mysql ...`, mert a `mysql`
      CLI nincs a hoszton telepítve): minden NEM összevont (`IsMerged=0`) ticketre, aminek nincs a
      `Body`-jával pontosan egyező `TicketMessage` sora, beszúrásra került egy `Direction=Inbound`
      üzenet `CreatedAt=Ticket.CreatedAt`-tal (hogy a szálban időrendben elsőként jelenjen meg) —
      17 ticket érintett (`#1-5, #8-10, #12-14, #19, #25-28, #29`). Az ÖSSZEVONT (`IsMerged=1`)
      ticketek (`#6, #7, #11, #15, #20`) SZÁNDÉKOSAN KI LETTEK HAGYVA a backfillből — ezeknél a
      Freshdesk-stílusú merge (20. lépés) már úgyis áthelyezte a valódi üzeneteket a cél ticketre, egy
      utólag a (lezárt) forrás ticketre backfillelt üzenet nem kerülne át oda és csak zavaró,
      "elárvult" tartalmat hozna létre a forrás ticket saját (amúgy nem használt) szálában
    - Tesztelve élő adaton: friss ticket létrehozása után azonnal 1 Inbound üzenet a `/messages`
      válaszban (majd törölve, teszt adat); backfill előtt/után ellenőrizve ticket #3 (0 → 1 üzenet)
      és #13 (a régi 6 outbound elé bekerült az eredeti inbound, helyes időrendi sorrendben)
  - `tsc -b`, `npm run build` (production) és `oxlint` (`MessageThread.tsx`,
    `TicketDetailPage.module.css`) tiszta — 0 warning. `dotnet build` tiszta, 0 warning/error. Backend
    háttérfolyamat újraindítva mindkét javításhoz (a friss C# kód betöltéséhez)
  - BÖNGÉSZŐS vizuális ellenőrzés a Fragment-javításhoz SEM történt ebben a környezetben (ugyanaz a
    Playwright-korlát) — a strukturális elemzés (flex-item lánc) és a `tsc`/build alapján magas
    bizonyossággal helyes, de a tényleges jobbra-igazítás böngészőben még nincs visszaigazolva
