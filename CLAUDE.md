...
30. SLA beállítások frontend újraépítése — Policy lista (táblázat, Master SLA badge, cég chipek, szerkesztés/törlés), Policy modal (DRY create/update: név, isDefault checkbox, businessHoursOnly checkbox, prioritás tábla ÓRÁBAN, cégek multi-select keresővel), amber figyelmeztető banner ha nincs default policy, Domain kivételek szekció ELTÁVOLÍTVA, backend UpdateSlaPolicyRequest kiegészítve isDefault mezővel (automatikusan unset-eli a többit), SlaStatusRow a TicketSidebar Tulajdonságok szekciójában (zöld/amber/piros badge, 30 mp-es refresh)
31. Sidebar tulajdonság csoportok, kártyanézet HTML fix, hover tooltip — TicketSidebar TULAJDONSÁGOK szekció 4 alcsoportra osztva (Státusz & Prioritás / Hozzárendelés / Kategorizálás / Egyéni mezők), szülő+alkategória dropdown logika (1 gyerek: readonly, >1: választható, 0: nincs alkategória), htmlToPlainText helper (lib/htmlText.ts, DOMParser alapú), kártyanézet lastMessageBody plain textre konvertálva (max 150 kar), Tooltip komponens (400ms delay, portal, --navy háttér) mindhárom nézetben a tárgy linkre hover
32. Ticket beállítások hub + aloldalak — /settings/tickets hub oldal (5 kártyás grid: Kategóriák/Tulajdonságok/Egyéni státuszok/CSM kezelés/Válaszsablonok), SettingsTicketCategoriesPage (kétszintű accordion, szülő+alkategória modal DRY), SettingsTicketPropertiesPage (custom fields, DnD), SettingsTicketTemplatesPage (canned responses), /settings/tickets/statuses → SettingsCustomStatusesPage, /settings/custom-statuses → redirect, submenu: "Egyéni státuszok" és "CSM kezelés" eltávolítva
33. Kontaktok és cégek a főmenübe — /contacts és /companies új route (RequireAuth + AppLayout, nem Settings), AppLayout sidebar: Kontaktok + Cégek navlink a Jegyek és Beállítások között, Viewer role: szerkesztés/törlés/létrehozás gombok rejtve (canEdit = role !== Viewer), /settings/contacts és /settings/companies → redirect az újra, TicketSidebar link frissítve /contacts-ra, submenu: Kontaktok + Cégek eltávolítva
...
38. 6 új dashboard widget — Backend: SlaBreakdown/RecentTickets/MyOpenTickets/CategoryBreakdown/AgentPerformance/CustomerActivity analytics endpointok + DTO-k, DashboardWidgetType enum kibővítve; Frontend: 6 új widget komponens (SlaBreakdownWidget PieChart, RecentTicketsWidget lista, MyOpenTicketsWidget SLA badge-el, CategoryBreakdownWidget vízszintes BarChart, AgentPerformanceWidget táblázat, CustomerActivityWidget BarChart kattintható), WidgetStorePanel kibővítve (AgentPerformance csak Admin/MasterAdmin), WidgetEditorPanel új config editorok (limit, timeRange, scope), DashboardPage widget switch kibővítve
39. Gmail/IMAP csatolmány + inline kép CID — Backend: InboundEmailAttachment record kibővítve (ContentId, IsInline), FileStorage entitás (MessageId nullable, TicketId FK, IsInline, ContentId), FileStorageConfiguration (SetNull/Cascade), AddInlineImageSupport migráció, ImapEmailService: BodyParts iteráció inline képekhez, MailpitEmailService: Inline lista feldolgozása HTTP API-ból, TicketEmailProcessor: ReplaceInlineImagesAsync (cid:→/api/portal URL, MinIO feltöltés), AttachmentService.UploadInlineAsync (POST /tickets/{id}/attachments/inline), BuildBodyWithInlineImagesAsync mindkét email service-ben (kimenő CID embed, named "internal" HttpClient), IFileStorageService+Program.cs kiegészítve; Frontend: @tiptap/extension-image + handleDrop/handlePaste a RichTextEditor-ban, ReplyComposer.handleImageUpload (uploadInline → downloadUrl), TicketDetailPage ticketId prop átadva, generated-client manuálisan bővítve (uploadInline metódus, InlineAttachmentResult)
40. Részletes kártyák UX, automatikus cégek — DetailedCard.module.css: jobb oldali gombok függőlegesen középre (.right align-self+justify-content), kuka gomb eltávolítva a részletes nézetből; CompanyService.UpsertByDomainAsync (személyes domain kizárás: gmail/yahoo/hotmail/stb., cégnév deriválás domainsegmentből), ContactService.UpsertAsync → UpsertByDomainAsync, BuildContactsFromTicketsAsync kiegészítve (meglévő kontaktok company nélkül is kapnak céget, companiesCreated visszaadva), SettingsCompaniesPage "Cégek generálása ticketekből" gomb; ContactService.DeleteAsync: soft-delete helyett hard-delete (ticket.ContactId nullázás ExecuteUpdateAsync-kal, majd delete)
41. Email threading, sablon változók, SLA fagyasztás, auto-responder — Backend: SendReplyEmailAsync Outlook-kompatibilis HTML előzmény blokk (korábbi outbound üzenetek, border-left quoted style); ITemplateService + TemplateService (string.Replace alapú változó motor, {{ticket.*}}/{{contact.*}}/{{agent.*}}/{{portal.url}}); SlaFreezeStatus entitás (sla_freeze_statuses tábla, seed: Pending=true, többi=false), SlaPausedAt nullable DATETIME a Ticket entitáson, UpdateStatusAsync SLA freeze/unfreeze logika (fagyasztáskor SlaPausedAt=now, felolvasztáskor SlaDueAt+=elapsed), SlaBreachChecker kizárja a fagyasztott ticketeket; AutoResponderTemplate entitás (auto_responder_templates tábla, seed: new_ticket trigger), TicketEmailProcessor auto-responder küldés új ticketnél; API: GET/PUT /api/portal/sla/freeze-statuses, GET/PUT /api/portal/settings/auto-responder; Program.cs ITemplateService; MailSettings PortalUrl mező; NSwag manuálisan bővítve (SlaFreezeStatusDto, UpdateSlaFreezeStatusesRequest, AutoResponderDto, UpdateAutoResponderRequest, SlaClient.getFreezeStatuses/updateFreezeStatuses, SettingsClient.getAutoResponder/updateAutoResponder); Frontend: SettingsSlaPage SlaFreezeSection toggle lista, SettingsEmailPage AutoResponderSection (toggle + subject/body textarea sablon változó tippekkel)
